class Story {
    ...
    comment(username, content, id) {
        ...
        let replyToComment = this._comments.find(c => c.Id === id);
        let nextReply = replyToComment.Replies.length + 1;
        let replyId = Number(`${replyToComment.Id}.${nextReply}`);
        let reply = {
            Id: replyId,
            Username: username,
            Content: content
    ...
    toString(sortingType) {
        ...
        for (const comment of comments) {
            let comStr = `-- ${comment.Id}. ${comment.Username}: ${comment.Content}`;
            let replyStr = comment.Replies
                .map(r => `--- ${r.Id}. ${r.Username}: ${r.Content}`)
                .join('\n');

            replyStr = comment.Replies.length > 0 ? `\n${replyStr}` : '';
            let combined = `${comStr}${replyStr}`;
            commentStrArray.push(combined);
        } 

        let fullCommentStr = this._comments.length > 0 
            ? `\n${commentStrArray.join('\n')}`
            : '';

        return `Title: ${this.title}
Creator: ${this.creator}
Likes: ${this._likes.length}
Comments:${fullCommentStr}`;
    }
}
